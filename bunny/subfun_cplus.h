#ifndef _SUBFUN_CPLUS_H
#define _SUBFUN_CPLUS_H
#include "comm_def.h"
...
namespace vic
{
	...
	//¼õµÍÑÕÉ«ÊýÄ¿ 256/N*256/N*256/N
	void colorReduce(const cv::Mat &image_in, cv::Mat &image_out, int div = 64);

	void sharpen(const cv::Mat &image_in, cv::Mat &image_out);

	void sharpen2D(const cv::Mat &image_in, cv::Mat &image_out);
	...
	void getHarris(const cv::Mat &image_in, cv::Mat &image_out);
}
