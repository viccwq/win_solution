...
/**  @function Erosion  */
void erosion(cv::Mat &src, cv::Mat &dst, int type, int size, cv::Mat el)
{
	int erosion_type;
	cv::Mat element;
...
/** @function Dilation */
void dilation(cv::Mat &src, cv::Mat &dst, int type, int size, cv::Mat el)
{
	int dilation_type;
	cv::Mat element;
...
void openClose(cv::Mat &src, cv::Mat &dst, int type, int size, int op, cv::Mat el)
{  
	int oc_type;
	cv::Mat element;
...
int getSizeContoursGray(cv::Mat &img_src, cv::Mat &img_dst, int area, int y_min, int y_max)
{  
	if(!img_src.data)  
	{  
		cout << "Fail to load image" << endl;  
